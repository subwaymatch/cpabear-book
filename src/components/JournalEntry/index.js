...
const parseJournalEntry = (code) => {
    ...
    for (const line of lines) {
        ...
        if (typeMatch) {
            ...
            const typeCode = typeMatch[2];
            accountType = accountTypeMap[typeCode];
        }
...
export default function JournalEntry({ code, }) {
    ...
    return (<div className={styles.journalContainer}>
      <div className={clsx(styles.journalGrid, hasDate ? styles.withDate : styles.withoutDate)}>
        {/* Header Row */}
        ...
        {/* Date cell */}
        {hasDate && (<div className={styles.dateCell} style={{ gridColumn: 1, gridRow: `2 / span ${entries.length}` }}>
            {date}
          </div>)}
        ...
        {entries.map((entry, index) => {
            ...
            const parts = new Intl.NumberFormat("en-US", options).formatToParts(entry.amount);
            const currencySymbol = isFirstDebit || isFirstCredit
            ...
            return (<React.Fragment key={index}>
              ...
              <div className={clsx([amountCellClasses, styles.debit])} style={{ gridRow: rowIndex }}>
                {!entry.isCreditRow && (<>
              ...
              <div className={clsx([amountCellClasses, styles.credit])} style={{ gridRow: rowIndex }}>
                {entry.isCreditRow && (<>
                    <span className={styles.currencySymbol}>
                      {currencySymbol}
                    </span>
