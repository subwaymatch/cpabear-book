...
graph LR
    ...
    B --> C["Develop Response Strategies"]
    C --> D["Monitor & Report"]
    D --> A
```
...
| **Avoid** | Exit the activity that gives rise to the risk | Gies Co. exits an unprofitable foreign market to avoid currency risk |
| **Reduce (Mitigate)** | Take action to reduce the likelihood or impact | MAS Inc. implements internal controls over financial reporting to reduce fraud risk |
...
**Example — Gies Co.:** Gies Co. has a $10,000,000 variable-rate loan at SOFR + 2%. Concerned about rising rates, Gies enters into a pay-fixed, receive-variable interest rate swap at 5%. If SOFR rises to 4%, Gies pays 5% fixed and receives 6% (SOFR + 2%) from the counterparty—netting an effective rate of 5%.
...
**Example — MAS Inc.:** MAS Inc. expects to receive €2,000,000 from a European customer in 90 days. The current spot rate is $1.10/€. MAS enters into a 90-day forward contract at $1.09/€, locking in proceeds of $2,180,000 regardless of future rate movements.
